...
class Lead extends PS_Controller
{
  public $menu_code = 'DBLEAD';
	public $menu_group_code = 'DB';
  public $menu_sub_group_code = '';
	public $title = 'Lead Master';

  public function __construct()
  {
    parent::__construct();
    $this->home = base_url().'masters/leads';
    $this->load->model('masters/lead_model');
  ...
  public function index()
  {
      ...
      'order_by' => get_filter('order_by', 'cust_order_by', 'code'),
		...
		$rows = $this->lead_model->count_rows($filter);
		//--- ส่งตัวแปรเข้าไป 4 ตัว base_url ,  total_row , perpage = 20, segment = 3
		$init	= pagination_config($this->home.'/index/', $rows, $perpage, $segment);
		$leads = $this->lead_model->get_list($filter, $perpage, $this->uri->segment($segment));


    $filter['data'] = $leads;

		$this->pagination->initialize($init);
    $this->load->view('masters/lead/lead_list', $filter);
  }
  ...
  public function add_new()
  {
    ...
    $tax = $this->lead_model->get_tax_group(getConfig('SALE_VAT_CODE'));
    $ds = array(
      'tax' => $tax,
      'properties' => $this->lead_model->get_properties_list(),
      'owner_name' => $this->employee_model->get_name($this->_user->emp_id)
    );

    $this->load->view('masters/lead/lead_add', $ds);
  }
  ...
  public function add()
  {
    ...
    if(!empty($data))
    {
      ...
      if(!empty($data->lead))
      {
        ...
        $cust['validFrom'] = empty(get_null($data->lead->validFrom)) ? NULL : db_date($data->lead->validFrom);
        $cust['validTo'] = empty(get_null($data->lead->validTo)) ? NULL : db_date($data->lead->validTo);
        $cust['frozenFrom'] = empty(get_null($data->lead->frozenFrom)) ? NULL : db_date($data->lead->frozenFrom);
        $cust['frozenTo'] = empty(get_null($data->lead->frozenTo)) ? NULL : db_date($data->lead->frozenTo);
      ...
      //--- insert Customer data table lead
      if($this->lead_model->add($cust))
      {
        //--- contact_person
        if(!empty($data->contactPerson))
        {
          foreach($data->contactPerson as $rs)
          {
            ...
            if(!$this->lead_model->add_contact($contactPerson))
            {
              $sc = FALSE;
              $this->error = "Add Contact Person Filed";
            }
          }
        }
        ...
        if(!empty($data->billTo))
        {
          foreach($data->billTo as $rs)
          {
            ...
            if(! $this->lead_model->add_address($billTo))
            {
              $sc = FALSE;
              $this->error = "Add BillTo Address Failed";
            }
          }
        }
        ...
        if(!empty($data->shipTo))
        {
          foreach($data->shipTo as $rs)
          {
            ...
            if(! $this->lead_model->add_address($shipTo))
            {
              $sc = FALSE;
              $this->error = "Add ShipTo Address Failed";
            }
          }
        }
        ...
        if($sc === TRUE)
        {
          ...
          if(getConfig('BP_MANUAL_EXPORT') != 1)
          {
            $this->doExport($code);
          }
        }
  ...
  public function edit($code)
  {
    ...
    $lead = $this->lead_model->get($code);

    if(!empty($lead))
    {
      $tax = $this->lead_model->get_tax_group(getConfig('SALE_VAT_CODE'));
      $contact = $this->lead_model->get_contact($lead->LeadCode);
      $billTo = $this->lead_model->get_bill_to($lead->LeadCode);
      $shipTo = $this->lead_model->get_ship_to($lead->LeadCode);
      $lead->project_name = $this->lead_model->get_project_name($lead->ProjectCod);
      $exists = $this->lead_model->is_sap_exists_code($code);
      ...
      $this->load->view('masters/lead/lead_edit', $ds);
    }
    else
    {
  ...
  public function update($code)
  {
    ...
    if(!empty($data))
    {
      ...
      if(!empty($lead))
      {
        if($lead->Status != 2)  //----- 2 = Already In SAP
        {
          //--- ตรวจสอบใน SAP ว่ามีอยู่จริงๆหรือเปล่า กันพลาด
          if(! $this->lead_model->is_sap_exists_code($code))
          {

            if(!empty($data->lead))
            {

              $LeadCode = $data->lead->LeadCode; //--- new lead code

              //--- drop temp
              if($this->lead_model->drop_temp_exists_data($code))
              {
                $cust = array();
                ...
                //--- insert Customer data table lead
                if($this->lead_model->update($code, $cust))
                {
                  //--- Drop current contact person
                  $this->lead_model->drop_contact_person($lead->LeadCode);
                  //--- Drop current billTo and ShipTo address
                  $this->lead_model->drop_address($lead->LeadCode);
                  ...
                  if(!empty($data->contactPerson))
                  {
                    foreach($data->contactPerson as $rs)
                    {
                      ...
                      {
                        $sc = FALSE;
                        $this->error = "Add Contact Person Failed";
                      }
                    }
                  }
  ...
  public function get_preview_data()
  {
    ...
    if(!empty($code))
    {
      $rs = $this->lead_model->get_preview_data($code);
      if(!empty($rs))
      {
        $arr = array(
          'U_WEBORDER' => $rs->code,
  ...
  public function doExport($code)
  {
    ...
    //--- check exists in SAP
    $exists = $this->lead_model->is_sap_exists_code($code); //-- web_code

    if(!$exists)
    {

      //--- lead data
      $rs = $this->lead_model->get($code); //--- web_code
      $web_order = $rs->code;
      $leadCode = $rs->LeadCode;

      //--- check temp
      $temp = $this->lead_model->is_temp_exists_data($rs->code); //-- web order

      if($temp)
      {
        //--- drop exists temp
        $this->lead_model->drop_temp_exists_data($rs->code); //--- web order
      }
      ...
      if(! $this->lead_model->add_sap_lead($arr))
      {
        $sc = FALSE;
      ...
      else
      {
        ...
        //--- contact person data
        $contact = $this->lead_model->get_contact($leadCode);
        ...
        if(!empty($contact))
        {
          foreach($contact as $ra)
          {
            ...
            if(! $this->lead_model->add_sap_contact_person($arr))
            {
              $sc = FALSE;
        ...
        //--- billTo address CRD1
        //--- bill to data
        $billTo = $this->lead_model->get_bill_to($leadCode);
        ...
        if(!empty($billTo))
        {
          foreach($billTo as $rs)
          {
            ...
            if(! $this->lead_model->add_sap_address($arr))
            {
              $sc = FALSE;
        ...
        //--- ShipTo Address CRD1
        //--- ship to data
        $shipTo = $this->lead_model->get_ship_to($leadCode);
        ...
        $this->lead_model->update($web_order, $arr);
      }
  ...
  public function get_temp_data()
  {
    ...
    $data = $this->lead_model->get_temp_data($code);
    if(!empty($data))
    {
      $arr = array(
  ...
  public function is_exists_code()
  {
    ...
    if($this->lead_model->is_exists_lead_code($leadCode, $code))
    {
      echo 'duplicate';
    }
    else
      ...
      if(!empty($cust))
      {
        if($cust->Status != 2)
        {
          if(! $this->lead_model->is_sap_exists_code($code))
          {
            ...
            {
              if(! $this->lead_model->delete($code, $cust->LeadCode))
              {
                $sc = FALSE;
                $this->error = "Failed : Delete lead data failed";
              }
            }
          }
          else
  ...
  public function get_running_number()
  {
    ...
    $run_digit = 4;
    $code = $this->lead_model->get_sap_max_code($pre);
  ...
  public function get_new_code($date = NULL)
  {
    ...
    $pre = $prefix .'-'.$Y.$M;
    $code = $this->lead_model->get_max_code($pre);
